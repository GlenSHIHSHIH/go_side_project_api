...
type User struct {
	...
	LoginName     string         `gorm:"comment:登入帳號;type:varchar(30);uniqueIndex" json:"loginName"`                      //登入帳號
	Password      string         `gorm:"comment:密碼;type:varchar(100)" json:"password"`                                    //密碼
	...
	LoginTime     time.Time      `gorm:"comment:最後登入時間;type:datetime;" json:"loginTime"`                                  //最後登入時間
