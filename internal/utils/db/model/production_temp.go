...
const (
	...
	INSERT_PRODUCTION = `insert into productions(product_id,name,description,options,categories,image,images,url,price,price_min,create_time,update_time)
							select 
								temp.product_id,temp.name,temp.description,temp.options,temp.categories,
								temp.image,temp.images,temp.url,temp.price,temp.price_min,temp.create_time,temp.update_time
